...
["LRM_Current_Loadout","save"] call aaf_fnc_gear; //Saving current loadout.
...
if (isNull _parent) then { //Check if player is NOT in a vehicle
	...
	if ((objectParent player) isKindof "StaticWeapon") then { //If they are in a vehicle, then checks if the vehicle is a static weapon.
