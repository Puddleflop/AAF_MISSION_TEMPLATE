...
{ 
	...
	if ((_owner == _playerUID) || (_owner == _playerObj)) then {
		...
		if ((getAssignedCuratorUnit _x) == player) then { 
			...
			systemChat "If you shouldn't have a zeus, screenshot the following, send to Tommo.";
			systemChat format ["%1: %2","_playerUID",_playerUID];
			systemChat format ["%1: %2","_playerObj",_playerObj];
			systemChat format ["%1: %2","player",player];
			systemChat format ["%1: %2","_owner",_owner];
			systemChat format ["%1: %2","Zeus module",_x];
